...
import NutritionFeedback from '../pageComponents/NutritionFeedback';
import NutrientDiet from '../pageComponents/NutrientDiet';
...
const RightSideBox = () => {
  ...
  const getLatestUpdatedDay = (data) => {
    let latestDay = null;
    let latestDate = null;
    for (const [day, details] of Object.entries(data)) {
      if (latestDate === null || new Date(details.lastUpdated) > new Date(latestDate)) {
        latestDate = details.lastUpdated;
        latestDay = { day, ...details };
      }
    }
    return latestDay;
  };

  const latestActivity = getLatestUpdatedDay(weeklyData);
      ...
      >
        ...
        {latestActivity ? (
          <div style={{ marginBottom: '10px' }}>
            <p><strong>{latestActivity.day}</strong></p>
            <p>Steps Taken: {latestActivity.stepsTaken || 'N/A'}</p>
            <p>Workout Done: {latestActivity.workout || 'N/A'}</p>
            <p>Duration: {latestActivity.workoutDuration || 'N/A'}</p>
          </div>
        ) : (
